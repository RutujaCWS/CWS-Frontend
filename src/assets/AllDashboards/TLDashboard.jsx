...
function TeamLeaderDashboard({ user }) {
  ...
  const fetchData = async () => {
    try {
      const leavesRes = await axios.get(
        `https://cws-backend-roan.vercel.app/leave/team-leader/${user._id}`,
      );
      setLeaves(leavesRes.data.leaves || []);
      
      const regRes = await axios.get(
        `https://cws-backend-roan.vercel.app/regularization/team-leader/${user._id}`,
      );
  ...
  const updateRegularizationStatus = async (attendanceId, status) => {
    try {
      ...
      await axios.put(
        `https://cws-backend-roan.vercel.app/attendance/regularization/${attendanceId}/status`,
        { status },
